...
CGFloat HEIGHT = 60.0; //%%% height of the segment
...
-(void)setupSegmentButtons {
    ...
    shopButton = [[UIButton alloc]initWithFrame:CGRectMake(X_BUFFER, Y_BUFFER, width, HEIGHT)];
    playGameButton = [[UIButton alloc]initWithFrame:CGRectMake(X_BUFFER+width, Y_BUFFER, width, HEIGHT)];
    listCardButton = [[UIButton alloc]initWithFrame:CGRectMake(X_BUFFER+2*width, Y_BUFFER, width, HEIGHT)];
    
    [toolbar addSubview:shopButton];
    [toolbar addSubview:playGameButton];
    [toolbar addSubview:listCardButton];
    
    shopButton.tag = 0;
    playGameButton.tag = 1;
    listCardButton.tag = 2;
    
    shopButton.backgroundColor = [UIColor colorWithRed:0.03 green:0.07 blue:0.08 alpha:1];
    playGameButton.backgroundColor = [UIColor colorWithRed:0.03 green:0.07 blue:0.08 alpha:1];
    listCardButton.backgroundColor = [UIColor colorWithRed:0.03 green:0.07 blue:0.08 alpha:1];
    
    [shopButton addTarget:self action:@selector(tapSegmentButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    [playGameButton addTarget:self action:@selector(tapSegmentButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    [listCardButton addTarget:self action:@selector(tapSegmentButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    
    [shopButton setTitle:@"Shop" forState:UIControlStateNormal];
    [playGameButton setTitle:@"Play" forState:UIControlStateNormal];
    [listCardButton setTitle:@"Cards" forState:UIControlStateNormal];
...
-(void)setupPageViewController {
    ...
    [pageController setViewControllers:@[[viewControllerArray objectAtIndex:1]] direction:UIPageViewControllerNavigationDirectionForward animated:YES completion:nil];
    [self syncScrollView];
}
...
-(void)updateSelectedItem{
    ...
    [playGameButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [listCardButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    
    shopButton.titleLabel.textColor = [UIColor whiteColor];
    playGameButton.titleLabel.textColor = [UIColor whiteColor];
    listCardButton.titleLabel.textColor = [UIColor whiteColor];
    
    switch (self.currentPageIndex) {
        case 0:
            [shopButton setTitleColor:[UIColor greenColor] forState:UIControlStateNormal];
            break;
        case 1:
            [playGameButton setTitleColor:[UIColor greenColor] forState:UIControlStateNormal];
            break;
        case 2:
            [listCardButton setTitleColor:[UIColor greenColor] forState:UIControlStateNormal];
            break;
        default:
            break;
    }
